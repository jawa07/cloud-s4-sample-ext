...
class MasterViewController: UITableViewController, SAPFioriLoadingIndicator{
    ...
    var productDescriptions = [AProductDescriptionType]()
    
    var selectedDetailProduct: AProductType = AProductType()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let headline = product.toDescription.filter({ $0.language == "EN" }).first?.productDescription {
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        let descriptionQuery = DataQuery().select(AProductDescriptionType.product, AProductDescriptionType.productDescription).filter(AProductDescriptionType.language.equal("EN"))
    ...
    func appendDescriptionsToProducts(products: [AProductType], descriptions: [AProductDescriptionType]) -> [AProductType] {
        for product in products {
            product.toDescription = descriptions.filter { $0.product == product.product && $0.language == "EN" }
        }
        return products
    }
