...
public class AddressDTOTest {
	...
	public void testAddressDTOSetsAllFieldsFromBusinessPartnerAddress() {
		...
		assertThat("validityStartDate", testee.getValidityStartDate(), 
			is(businessPartnerAddress.getValidityStartDate()));
		assertThat("validityEndDate", testee.getValidityEndDate(), 
			is(businessPartnerAddress.getValidityEndDate()));
	...
	/**
	 * Helper method to generate the test method
	 */
	@Test
	...
	/**
	 * Helper method to generate {@link 
	 * 
	 */
	@Test
	public void generateToBusinessPartnerAddress() {
		final Field[] fields = AddressDTO.class.getDeclaredFields();
		
		final String constructor = Arrays.asList(fields).stream()
			.filter(field -> !Modifier.isStatic(field.getModifiers())) // filter out static fields 
			.map(Field::getName) // get field names
			.map(name -> String.format("\tthis.%s = businessPartnerAddress.get%s();", 
					name, 
					upperCaseFirstChar(name)))
			.collect(Collectors.joining("\n"));
		
		System.out.println(String.format("public %s(BusinessPartnerAddress businessPartnerAddress) {", 
				AddressDTO.class.getSimpleName()));
